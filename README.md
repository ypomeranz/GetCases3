...
When the opinion in that window carries **separate writings**, a slim rail appears just inside the scrollbar mapping them: each part covers the stretch of the document it occupies, in its own color (blue for the Court’s opinion, green for a concurrence, red for a dissent, grey for a syllabus), with a solid marker on its first line. Dragging the scrollbar you can see how far down the dissent starts; hovering names the part and its page, and clicking jumps straight to it. **Taking hold of the scrollbar names them all at once** – every writing's author in caps, in the part's own colour, set directly to the left of the band that writing begins at, the Court's own opinion among them (“ROBERTS”, “KAGAN”, “THOMAS”; “PER CURIAM” for an opinion in the Court's name, and the kind of writing where the heading names nobody). A syllabus is nobody's writing and is left unnamed. The names come up on the press and stay for as long as the thumb is held – the wheel over the scrollbar and a click on the rail itself bring them up too – then fade back into the page a second or so after the scrolling stops, so nothing is left sitting over the opinion. Reaching past one with the pointer takes them down at once rather than swallowing the click. The parts come from the same detector the **Side panel** parts list uses, and the rail appears only for an opinion that actually has a concurrence, dissent or other separate writing – a lone majority gets no rail.
