...
**A Supreme Court case leads with its merits opinion.** Search a recent Supreme Court caption and Scholar returns the whole family filed under it: the merits opinion, and the orders along the way – the grant of certiorari, a stay (sometimes with a dissent), argument and briefing motions, a recall of the judgment. Neither Scholar’s order nor its citation counts reliably put the merits opinion first (the stay in *Trump v. Slaughter*, with its dissent, is cited several times as often as the merits opinion that followed it), so an order is recognized by what its snippet says – the Reporter’s heading with a bare date where an opinion’s reads “Argued … Decided …”, or the order’s own words – and the merits opinion moves to the head of its case’s rows and above the decisions below it. Several cases sharing one caption (“United States v. Texas”) keep Scholar’s order, and a writing Scholar lists twice (slip and reported) is shown once. Saved Supreme Court orders and CourtListener’s Supreme Court rows, which often answer before Scholar, wait for Scholar’s rows (for six seconds at most) so they cannot take the top of the list; a saved merits opinion is shown at once.
